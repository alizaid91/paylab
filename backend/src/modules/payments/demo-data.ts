...
import { customers, paymentAttempts, payments } from "../../db/schema.js";

type DbTransaction = Parameters<Parameters<typeof db.transaction>[0]>[0];

const names = [
  "Aarav Mehta", "Isha Kapoor", "Rohan Shah", "Ananya Iyer", "Vikram Rao",
  "Neha Patel", "Kabir Singh", "Meera Nair", "Arjun Das", "Tara Joshi",
  "Dev Malhotra", "Priya Menon", "Aditya Verma", "Sana Khan", "Yash Thakur",
  "Nandini Bhat", "Kunal Bansal", "Riya Sethi", "Manav Gupta", "Aditi Kulkarni",
  "Siddharth Jain", "Pooja Reddy", "Rahul Sood", "Kavya Pillai", "Nikhil Arora",
  "Simran Kaur", "Varun Chawla", "Shruti Desai", "Harsh Vardhan", "Mahi Shah",
  "Ankit Tandon", "Ishita Bose", "Om Prakash", "Lavanya Roy", "Gaurav Khanna",
  "Divya Shetty", "Ritesh Nair", "Ayesha Mirza", "Mohit Saxena", "Tanvi Ghosh",
  "Abhishek Dutta", "Sonal Kapoor", "Rajiv Menon", "Nisha Thomas", "Pranav Rane",
  "Sneha Bedi", "Aman Oberoi", "Kritika Paul", "Suresh Iyer", "Reema Mathew",
  "Vivek Narang", "Pallavi Rao", "Sameer Kohli", "Juhi Agarwal", "Deepak Yadav",
  "Alisha Fernandes", "Raghav Bhatia", "Monika Wadhwa", "Naveen Joshi"
] as const;
...
export async function generateDemoData(
  ...
  merchantId: string,
  seed = Date.now()
): Promise<{ customers: number; payments: number; paymentAttempts: number }> {
  ...
  const seededCustomers = await tx.insert(customers).values(customerRows).returning();
  ...
  for (const customer of seededCustomers) {
    ...
    const transactionCount = PAYMENT_COUNT_PER_CUSTOMER.min
      + Math.floor(random() * (PAYMENT_COUNT_PER_CUSTOMER.max - PAYMENT_COUNT_PER_CUSTOMER.min + 1));
    for (let customerPayment = 0; customerPayment < transactionCount; customerPayment += 1) {
      ...
      paymentRows.push({
        ...
        metadata: {
          ...
          failureReason: failed
            ? eveningUpiFailure ? "upi_timeout"
              : mobileCardFailure ? "issuer_decline"
                : repeatFailure ? "customer_retryable_decline" : "provider_decline"
            : null,
          seedPattern: eveningUpiFailure ? "upi_evening_failure"
            : mobileCardFailure ? "mobile_card_failure"
              : repeatFailure ? "repeat_customer_failure" : "baseline"
        }
      });
      ...
      for (let attempt = 1; attempt <= attemptCount; attempt += 1) {
        const attemptTime = new Date(createdAt.getTime() + (attempt - 1) * 90_000);
        attemptRows.push({
          merchantId,
          paymentId,
          ...
          errorCode: failed || attempt < attemptCount
            ? eveningUpiFailure ? "U16" : mobileCardFailure ? "05" : "DO_NOT_HONOR"
            : null,
          errorMessage: failed || attempt < attemptCount
            ? "Payment was declined by the payment provider" : null,
  ...
  return {
    ...
    paymentAttempts: attemptRows.length
  };
}
