...
export class ExecutionService {
  async listForUser(userId: string) {
    ...
    return db
      .select({
        ...
        strategy: { id: strategies.id, name: strategies.name },
      })
      .from(executions)
      .leftJoin(
        executionResults,
        eq(executionResults.executionId, executions.id),
      )
      .innerJoin(strategies, eq(strategies.id, executions.strategyId))
      .where(eq(executions.merchantId, merchantId))
      .orderBy(desc(executions.createdAt));
  }

  async getByIdForUser(userId: string, executionId: string) {
    const merchantId = await merchantIdForUser(userId);
    const [record] = await db
      ...
      .where(
        and(
          ...
          eq(executions.merchantId, merchantId),
        ),
      )
      .limit(1);
    if (!record)
  ...
  async approveForUser(userId: string, strategyId: string) {
    ...
    return db.transaction(async (tx) => {
      ...
      if (strategy.status !== "policy_approved") {
        throw new AppError(
          409,
          "STRATEGY_NOT_APPROVABLE",
          "Strategy must pass policy checks before merchant approval",
        );
      }
      ...
      if (!advisory)
        throw new AppError(
          ...
          "An advisory review is required",
        );
      ...
      if (!policy)
        throw new AppError(
          ...
          "A passed policy check is required",
        );
      ...
      if (!approved)
        throw new AppError(
          409,
          "STRATEGY_NOT_APPROVABLE",
          "Strategy approval state changed; retry the request",
        );
  ...
  async executeForUser(userId: string, strategyId: string) {
    ...
    return db.transaction(async (tx) => {
      ...
      if (
        strategy.status !== "merchant_approved" ||
        !strategy.approvedByUserId
      ) {
        throw new AppError(
          409,
      ...
      if (existing)
        throw new AppError(
          409,
          "DUPLICATE_EXECUTION",
          "This strategy has already been executed",
        );
      ...
      const expectedRecovery =
        stringValue(output.potentialRevenueRecovery) ?? "0";
      ...
      if (!result)
        throw new AppError(
          ...
          "Unable to create execution result",
        );
