...
function ExecuteContent({ id }: { id: string }) {
  ...
  const execute = () => {
    if (window.confirm("Are you sure you want to execute this strategy?"))
      execution.mutate();
  };
    ...
    <ContentContainer>
      ...
      {isApproved && (
        <section className="mt-8 rounded-lg border border-accent/20 bg-accent/5 p-6">
          <h2 className="text-lg font-semibold">Execute Strategy</h2>
          <p className="mt-1 text-sm text-muted-foreground">
            Execution is simulated in the MVP and will use the approved
            strategy.
          </p>
          <Button
            className="mt-5"
            onClick={execute}
            disabled={execution.isPending || execution.isSuccess}
          >
            {execution.isPending && (
              <Loader2 className="mr-2 h-4 w-4 animate-spin" />
            )}
          ...
          {execution.isError && (
            ...
            >
              ...
              Unable to execute this strategy. Please verify merchant approval
              and policy status.
            </div>
          )}
          {execution.data && (
    ...
    <div className="mt-6 space-y-5">
      <div className="flex items-center gap-2 text-emerald-700">
        ...
        <p className="font-semibold">
          Execution Status: {title(execution.status)}
        </p>
      </div>
      <div className="grid gap-4 sm:grid-cols-3">
        <SummaryCard heading="Transactions Affected">
          <strong>
            {execution.affectedTransactionCount.toLocaleString("en-IN")}
          </strong>
        </SummaryCard>
        <SummaryCard heading="Expected Recovery">
          <strong>{money(execution.expectedRecovery)}</strong>
        </SummaryCard>
        <SummaryCard heading="Actual Simulated Recovery">
          <strong className="text-accent">
            {money(result?.actualRecovery)}
          </strong>
        </SummaryCard>
      </div>
