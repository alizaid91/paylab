...
export function LoginForm() {
  ...
  const mutation = useMutation({ mutationFn: login, onSuccess: (data) => { window.sessionStorage.setItem("paylab_access_token", data.accessToken); queryClient.setQueryData(["auth", "me"], { user: data.user, merchant: data.merchant }); router.replace("/payments"); } });
...
export function RegisterForm() {
  ...
  const mutation = useMutation({ mutationFn: (values: RegisterValues) => register({ email: values.email, password: values.password, merchant: { name: values.merchantName, slug: values.merchantName.toLowerCase().trim().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "") || "merchant", defaultCurrency: "USD", timezone: "UTC" } }), onSuccess: (data) => { window.sessionStorage.setItem("paylab_access_token", data.accessToken); queryClient.setQueryData(["auth", "me"], { user: data.user, merchant: data.merchant }); router.replace("/payments"); } });
